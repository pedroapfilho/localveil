...
There is no account and no server. Once the model is cached, the page works offline.

**At a glance**

- **What it is:** a browser app that redacts personal data from files locally.
- **What it takes:** plain text, Markdown, CSV, JSON, logs, PDFs, and images.
- **What it gives back:** the same files with the personal data covered, in a ZIP.
- **Where it runs:** entirely in the tab. Nothing is uploaded, and the only network traffic is the one-time model download.
...
`@repo/redact-core` owns the shared vocabulary: what a span is, how text is chunked for the model, how spans become rectangles, and how the ZIP is built. Every redactor implements the same `Redactor` shape, so the worker resolves one from the file and knows nothing else about it. Adding a format means adding a package, not editing the worker.

## Packages

| Package              | Purpose                                                                                           |
| -------------------- | ------------------------------------------------------------------------------------------------- |
| `@repo/redact-core`  | Types, text chunking, span merging, span-to-rectangle mapping, repeat matching, ZIP building.     |
| `@repo/pii-detect`   | Token classification over `openai/privacy-filter`, BIOES decoding, and the resumable model cache. |
...
| `@repo/ui`           | Shared components: dropzone, progress, button, card, toaster.                                     |

`@repo/typescript-config` and `@repo/config-vitest` hold the shared tooling config.
...
The file is read as a string, chunked with an overlap so an entity split across a boundary is still seen whole, and each chunk goes to the model. Spans come back as character ranges, get merged, and each covered grapheme becomes `█`, so an emoji or an accented letter takes one block rather than two.
...
<details>
<summary><b>Language</b>: detected, never configured</summary>
...
Detection scores stopword frequency across the three supported languages, written without accents because the English probe drops most of them.
