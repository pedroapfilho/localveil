...
Run `pnpm lint` after changes and fix every error. `oxlint.config.ts` registers `@shadcn/lint` and enforces all six rules as errors: component contracts, known Tailwind classes, static component class names, semantic colors, theme or scale values, and class-based styling. Use CSS custom properties for runtime geometry and named theme tokens for custom values. Primitives in `packages/ui/src/components` own their internal styles. Preserve upstream component APIs; product-specific status text, file picking, and fractional progress belong in compositions. `apps/web/components.json` resolves the shared theme, including `scroll-fade` and `no-scrollbar`. The exact `foo`, `bar`, and `baz` allowances apply only to the class-merging test fixture.
